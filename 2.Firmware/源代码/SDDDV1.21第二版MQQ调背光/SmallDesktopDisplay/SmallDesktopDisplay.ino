...
char tftdisplay = 1;////反转所有显示颜色：1反转，0正常
...
String publishTopicName = "sdddlog" ;
String subscribeTopicName = "sddder" ;
...
void setlcdlv(){
  if(web_lcdbl>0 && web_lcdbl<=100)
  {
    ...
    //TODO
    String result = "写入亮度值成功：" + String(web_lcdbl);
    Serial.print("等待推送消息：");
    Serial.println(result);
    Serial.print("等待推送消息长度：");
    Serial.println(result.length() + 1);
    pubMQTTmsg(result ); //调用函数发布消息
  }
}
...
void connectMQTTServer(){
    ...
    String clientId = "c30b4a86-87db-46c8-abd5-2569fbd122c3";
	  String macadd = WiFi.macAddress();

    if(waitNum<10){
        if (mqttClient.connect(clientId.c_str())) { 
        Serial.println("MQTT服务连接成功...");
        Serial.print("服务地址:");
        Serial.println(mqttServer);
        Serial.print("客户端Id:");
        Serial.println(clientId);
        Serial.print("客户端Mac地址:");
        Serial.println(macadd);
        subscribeTopic(); // ****订阅指定主题****
      } else {
        Serial.print("MQTT服务连接失败，当前状态:");
...
// 发布信息
void pubMQTTmsg(String message){

     
    // 建立发布主题
    char publishTopic[publishTopicName.length() + 1];//读取发布主题名称长度 扩容（长度+1） 
    strcpy(publishTopic, publishTopicName.c_str());//发布主题名称复制给publishTopic
   
    // 建立发布信息:设备的状态    
    char publishMsg[message.length() + 1]; //获取消息内容长度 扩容（长度+1） 
    strcpy(publishMsg, message.c_str());//Copy消息内容给发布消息
    ...
    if(mqttClient.publish(publishTopic, publishMsg)){
      ...
      Serial.print("主题:");
      Serial.println(publishTopicName);
      Serial.print("消息内容");
...
void receiveCallback(char* topic, byte* payload, unsigned int length) {
    ...
    if(json["brightness"]>=0){
      web_lcdbl  = json["brightness"].as<int>();
      //Serial.print("JSON接口转换得亮度：");
      //Serial.println(web_lcdbl); 
      //web_lcdbl = msg.toInt(); 
      setlcdlv(); 
      Serial.println("变更亮度完成..."); 
    }
    if(json["tftdisplay"]>=0){
      tftdisplay = json["tftdisplay"];

     
      tft.invertDisplay(tftdisplay);//反转所有显示颜色：1反转，0正常
      //tft.fillScreen(0x0000);
      //tft.setTextColor(TFT_BLACK, bgColor);
      
      
      Serial.println("变更主题完成..."); 
    }
